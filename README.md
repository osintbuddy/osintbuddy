...
[osib-demo-2025-10-07.webm](https://github.com/user-attachments/assets/a3656561-36f8-40fb-bfd3-4c65b6fed3d0)
...
>
> OSINTBuddy is currently experimental software. It's not quite ready for use _yet_! If you're interested in development you'll need the `plugins` docker service cloned: `git clone https://github.com/osintbuddy/plugins osintbuddy-plugins` to the root of this repo
...
| -------------------------------------------------------- | ---------------------------------------- | ----------------------- | ------------------------------------ | --------------------- |
| [`osintbuddy`](https://github.com/osintbuddy/osintbuddy) | The core application _(this repo)_       | Rust, TypeScript/Preact | Web interface and backend services   | 2-4 months from alpha |
...
- [Python 3.12.9](https://github.com/pyenv/pyenv)
- [Rust](https://rustup.rs/)
- [Firecracker _(prod only)_](https://firecracker-microvm.github.io/)
...
- **Configure frontend (optional):** Copy `frontend/.env.example` to `frontend/.env`, adjust as needed.
  - ```bash
    ...
    ```
- **Configure Casdoor:** Copy the `.casdoor/conf.example` directory to `.casdoor/conf`,
  adjust for your needs or use the provided configuration files for development.

  - ```bash
    cp -r .casdoor/conf.example/ .casdoor/conf
    ```
...
This Rust rewrite exists because I got tired of Python and I decided to rewrite the entire thing to learn Rust. Now it's faster and crashes less _(hopefully)_.
...
3. [**Initialize default entities** _(aka plugins)_](https://github.com/osintbuddy/plugins/blob/main/src/osintbuddy/ob.py#L85) for OSIB:
   ...
   ```sh
   # we are using yarn, to install:
   # npm install --global yarn
   cd frontend/ && yarn && cd ..

   git clone https://github.com/osintbuddy/plugins.git plugins-system

   python3 -m venv venv
   source ./venv/bin/activate

   pip install -e ./plugins-system/
...
4. **Start [the stack](https://github.com/osintbuddy/osintbuddy/blob/main/docker-compose.yml)**.
   ...
   ```
   docker compose up db ui casdoor
   ```
...
- **To delete _all_ OSINTBuddy data**: Data removal is an **irreversible** operation! **DO NOT** run this command if you do not want to lose **all** of your data! To remove all the data stored in your OSINTBuddy server run `docker compose down --volumes`.
...
- **Note**: _the [OSINTBuddy PyPi package](https://github.com/jerlendds/osintbuddy-plugins) is MIT licensed. We understand some individuals and businesses may not want to share their custom plugins._
