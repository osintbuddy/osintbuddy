...
export default function WorkspacePage() {
  ...
  const [activeEntity, setActiveEntity] = useState<DropdownOption>({ label: 'Select entity...' });
  ...
  const sortedEntities = query === '' || query?.includes('Select') || query === null
    ? entities.slice().sort((a: any, b: any) => new Date(b.last_edit) - new Date(a.last_edit))
    : entities.filter((e: any) => e?.label?.toLowerCase().includes(query.toLowerCase()));
    ...
    <>
      <div class="flex flex-col w-full pt-2.5 px-3">
        <section class="flex items-center relative rounded-lg  h-min justify-between z-[99]">
          {/* Custom Combobox */}
          <div class="relative  max-w-xs border-b backdrop-blur-md from-black/30 to-black/45 bg-gradient-to-br border-mirage-700/20 ">
                ...
                {entities?.length !== 0 ? (
                  sortedEntities.map((entity, index) => (
                    <div
                      ...
                      class={`text-nowrap px-4 flex-col hover:bg-black border-l-2 border-transparent hover:border-primary-400 flex py-1.5 nowheel nodrag cursor-default select-none transition-colors ${focusedIndex === index ? 'bg-black/50 text-slate-350 border-primary-400' : 'text-slate-400'
                        }`}
                      role="option"
