...
export function Header({ title, header, btnTxt, btnAction }: PageHeaderProps) {
  return (
    <div className='relative mx-auto flex sm:px-2 lg:px-4 justify-center w-full'>
      <div className='min-w-0 max-w-2xl flex-auto pt-3 lg:max-w-none lg:pr-0 px-2'>
        <section className='space-y-1'>
          {title && (
            <h4 className='font-display text-sm font-medium text-primary-200'>
              {title}
            </h4>
          )}
          {header && (
            <p className='font-display leading-4 text-xl tracking-tight text-slate-400 dark:text-slate-300'>
                {header}
            </p>
          )}
        </section>
