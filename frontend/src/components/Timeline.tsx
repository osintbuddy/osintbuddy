...
export function Timeline({ title, releaseDate, description, downloadLink }: TimelineProps) {
    return (
        <li class="ml-6">
            <span class="absolute flex items-center justify-center w-8 h-8 bg-blue-100 rounded-full -left-3 ring-8 ring-white dark:ring-mirage-800/30 dark:bg-primary-350">
            ...
            <h3 class="flex items-center mb-1 text-xl font-semibold text-gray-900 dark:text-slate-300"> {title}
                {downloadLink && <span class="bg-blue-100 text-blue-800 text-sm font-medium mr-2 px-2.5 py-0.5 rounded dark:bg-primary-350 dark:text-radiance-50 ml-3">Latest</span>}
            </h3>
            ...
            {downloadLink && (
                <Button.Ghost variant="primary" className='max-w-xs min-w-min'>
                    <a href={downloadLink} className="!text-slate-350">Download Source Code</a>
