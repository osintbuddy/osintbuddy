...
export function Timeline({
  ...
  return (
    <li class='ml-6'>
      ...
      <h3 class='flex items-center mb-1 text-xl font-semibold text-gray-900 dark:text-slate-300'>
        {' '}
        {title}
        {downloadLink && (
          <span class='text-sm font-medium mr-2 px-2.5 py-0.5 rounded bg-primary-350 text-radiance-50 ml-3'>
            Latest
          </span>
        )}
      </h3>
      ...
      {downloadLink && (
        <Button.Ghost variant='primary' className='max-w-xs min-w-min !my-4'>
          <a
            href={downloadLink}
            className='!text-inherit !font-display !text-sm'
          >
